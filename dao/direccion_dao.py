...
class DireccionDAO:
    ...
    @staticmethod
    def getDirecciones():
        query = 'SELECT id,calle,numero_calle,pais,ciudad,codigo_postal,latitud,longitud FROM Direccion ORDER BY id'
        results = Database.execute_query(query)
        return results if results else []
    
    @staticmethod
    def getDireccionById(id):
        query = "SELECT id, calle, numero_calle, pais, ciudad, codigo_postal, latitud, longitud FROM Direccion WHERE id = %s"
    ...
    def post(direccion):
        ...
        query = """
            INSERT INTO Direccion (calle, numero_calle, pais, ciudad, codigo_postal, latitud, longitud) 
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        params = (direccion["calle"], direccion["numero_calle"], direccion["pais"],
                 direccion["ciudad"], direccion.get("codigo_postal"), 
    ...
    def patch(direccion_id, direccion_args):
        ...
        query = """
            ...
            SET calle = %s, numero_calle = %s, pais = %s, ciudad = %s, 
                codigo_postal = %s, latitud = %s, longitud = %s 
            WHERE id = %s
        """
        params = (direccion_args["calle"], direccion_args["numero_calle"], 
                 direccion_args["pais"], direccion_args["ciudad"], 
                 direccion_args.get("codigo_postal"), direccion_args["latitud"], 
                 direccion_args["longitud"], direccion_id)
